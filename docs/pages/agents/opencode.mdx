...
See [opencode.ai](https://opencode.ai) for other install methods.

## Configure

Add to your global config (`~/.config/opencode/opencode.json`) or project-level `opencode.json`:

```jsonc [opencode.json]
// [!include ~/snippets/setup/opencode.jsonc]
```

The `solidity-language-server` binary must be in your `$PATH` (installed via `cargo install solidity-language-server`).
...
```sh
opencode run "use the lsp tool to hover on src/Pool.sol at line 102 col 15"
```
...
```sh
opencode run "use the lsp tool to find all references to the initialize function in src/PoolManager.sol"
```

## Server settings

Pass initialization options to configure server behavior:
